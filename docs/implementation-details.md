# 現状ロジック詳細

このドキュメントは、`spectral-packing` CLIの現状実装を説明するものです。対象は主に `src/main.rs` のパッキング本体、`scripts/fetch_thingi10k_cases.py` のThingi10K取得処理、`scripts/validate_thingi10k_tight.py` のbboxタイト検証処理です。GitHub Pagesの公開運用は [`deployment.md`](deployment.md) に分けています。

実装は、論文「汎用3D物体の高密度・非噛み込み・スケーラブルなスペクトルパッキング」の配置パイプラインをベースにした、制約充足型の3Dパッカーです。論文実装より高速であることではなく、指定されたトレイ境界、物体同士の非衝突、外側からの到達可能性など、実装が扱う制約モデルを破らない配置だけを出力することを目的にしています。

## 実装の目的と保証範囲

このリポジトリの意図は、アルゴリズムを眺めるための確認用実装ではありません。性能が論文実装に劣っていても、パック済みとして出力する解が制約違反を含まないことを優先します。

ただし、現状の実装には「厳密に扱っている制約」と「まだ幾何的に厳密化が必要な制約」があります。前者は出力時に破らない前提の制約です。後者は、現在は保守ボクセル化やAABBによる安全側のモデルで扱っており、今後の改善対象です。

| 区分 | 制約 | 現状の扱い |
| --- | --- | --- |
| 厳密に満たす対象 | トレイ境界 | 離散探索時とrefinement時にトレイ外へ出る候補を採用しない |
| 厳密に満たす対象 | 三角形が通過する表面ボクセルの占有 | 三角形とボクセルAABBの交差判定で、通過セルを必ずmarkする |
| 厳密に満たす対象 | 離散ボクセル上の物体同士の非重複 | FFT相関の衝突数がゼロの候補だけを採用する |
| 厳密に満たす対象 | 離散オフセット空間での外側到達可能性 | Flood-fillで境界から到達できる候補だけを採用する |
| 厳密に満たす対象 | refinement時の保守的な非衝突 | 三角形AABBが安全余白込みで重なる候補を採用しない |
| 厳密化が必要 | 任意回転を含む実際の分解経路 | 現状は6軸直線ray解析。回転や斜め方向の経路は未評価 |
| 厳密化が必要 | 非watertightメッシュの内部推定 | 現状は保守的な表面ボクセルと外部Flood-fillに依存する |

## 全体像

```mermaid
flowchart TD
    A[STL入力<br>ファイルまたはディレクトリ] --> B[STL読み込み<br>ASCII/Binary]
    B --> C[メッシュをbbox体積の降順にソート]
    C --> D[各物体を順に配置]
    D --> E[90度姿勢集合を列挙<br>最大24姿勢]
    E --> F[姿勢ごとにボクセル化]
    F --> G[FFT相関で衝突数と近接スコアを計算]
    G --> H{衝突なし?}
    H -- いいえ --> E
    H -- はい --> I{Flood-fill到達可能?}
    I -- いいえ --> E
    I -- はい --> J[コスト最小候補を採用]
    J --> K[連続サブボクセルrefinement]
    K --> L[占有ボクセルへstamp]
    L --> M{未処理物体あり?}
    M -- はい --> D
    M -- いいえ --> N[ray-casting分解可能性解析]
    N --> O{噛み込み候補あり?}
    O -- はい --> P[候補グループを取り外し・再挿入]
    P --> N
    O -- いいえ --> Q[結合ASCII STLと統計を出力]
```

パッキングは完全な最適化問題としては解かず、物体を1つずつ固定していく貪欲法です。各物体については、トレイ内の全平行移動候補をFFT相関で評価するため、単純な全候補・全ボクセル比較より高速に配置候補を探します。

## CLIと入出力

| コマンド | 役割 |
| --- | --- |
| `spectral-packing sample` | 手続き生成した小さなSTLセットを作成する |
| `spectral-packing pack` | STL群を1つのトレイへパックし、結合ASCII STLを出力する |

`pack` は次の入力を受け取ります。

| 入力 | 内容 |
| --- | --- |
| STLファイルまたはディレクトリ | ディレクトリ指定時は直下の `.stl` を名前順に読む |
| `--width`, `--depth`, `--height` | トレイ寸法 |
| `--voxel` | ボクセルの辺長 |
| `--rotations` | 試す姿勢数。24までは90度刻み、24超では追加の角度姿勢も試す |
| `--height-weight` | 上方向に積むことへのペナルティ |
| `--strategy` | `spectral` または `order-bl`。後者は積み込み順探索とBL前線候補で配置 |
| `--beam-width` | 残す部分配置候補数。1なら従来の貪欲配置 |
| `--order-window` | `order-bl` で次に積む候補として見る未配置物体数 |
| `--bl-candidate-limit` | `order-bl` で評価するBL前線候補の上限 |
| `--refine-margin` | refinement中の三角形AABBクリアランス |
| `--post-opt-passes` | 取り外し・再挿入後処理の最大パス数 |
| `--repack-passes` | 未配置物体を入れるための局所再パック最大パス数 |
| `--repack-window` | 局所再パックで一度に外す配置済み物体数 |
| `--repack-unpacked-limit` | 局所再パックで一度に試す未配置物体数 |
| `--time-limit-seconds` | 指定秒数を超えたら部分結果を出力して打ち切る |
| `--no-repack` | 未配置物体向け局所再パックを無効化 |
| `--no-post-opt` | 取り外し・再挿入後処理を無効化 |
| `--no-refine` | サブボクセル補正を無効化 |
| `--no-interlock` | Flood-fill到達可能性フィルタを無効化 |
| `--no-ray-disassembly` | ray-casting分解可能性解析を無効化 |

出力は、配置に成功した物体の三角形を1つにまとめたASCII STLです。あわせて、パック数、ボクセル密度、メッシュ密度、各物体のオフセット・平行移動量・refinement量・姿勢番号、ray分解判定を標準出力へ表示します。時間制限に達した場合もエラー終了にはせず、その時点で配置済みの物体だけを部分結果として書き出します。

## データ構造

| 構造体 | 主な役割 |
| --- | --- |
| `Vec3` | 3次元ベクトル。加減算、内積、外積、長さ、min/maxを提供 |
| `Mesh` | STLから読んだ三角形列と名前 |
| `Rotation` | 90度刻みの右手系回転行列 |
| `OrientedMesh` | 特定姿勢に回転・正規化し、ボクセル化済みの物体 |
| `Tray` | トレイ寸法、ボクセルサイズ、グリッド分割数 |
| `Placement` | 探索中の配置候補。姿勢、オフセット、コストを保持 |
| `PlacedMesh` | 採用済み物体。変換済み三角形、占有セル、統計値を保持 |
| `Deadline` | `--time-limit-seconds` の終了時刻を保持し、長い探索ループから参照する |
| `PackResult` | 配置成功・失敗リスト、ray分解レポート、時間打ち切り有無を保持 |

内部の3次元配列は、`idx(x, y, z, nx, ny) = (z * ny + y) * nx + x` で1次元配列へ格納します。

## STL読み込みとサンプル生成

STL読み込みはASCII/Binary両対応です。Binary STLは80バイトヘッダ後の三角形数とファイル長が一致するかで判定し、各三角形の法線は読み飛ばして頂点だけを使用します。ASCII STLは `vertex` 行を集め、3頂点ごとに1三角形として扱います。

手続き生成サンプルでは、直方体、L字ブロック、U字ブリッジ、三角柱、円柱、星形柱を三角形リストとして作り、ASCII STLへ書き出します。これは外部データに依存せずに制約処理の基本挙動を確かめるための最小サンプルです。

## 姿勢生成

姿勢は、座標軸の置換と符号反転から3x3行列を作り、行列式が `1` のものだけを残します。これにより、反転を含まない右手系の90度刻み姿勢を最大24個得ます。`--rotations` に24を超える値を指定した場合は、45度・30度・60度を含むEuler角の追加候補も決定的に生成します。

```mermaid
flowchart LR
    A[軸の置換 6通り] --> C[符号の組み合わせ 8通り]
    C --> D[3x3行列を生成]
    D --> E{determinant = 1?}
    E -- はい --> F[右手系姿勢として採用]
    E -- いいえ --> G[鏡像反転なので破棄]
    F --> H[重複除去]
    H --> I[--rotationsの数まで使用]
```

既定値では箱詰めで効果が出やすい軸揃え姿勢を優先します。追加角度姿勢は探索空間を広げるため遅くなりますが、90度姿勢だけでは通らない向きの候補を試せます。

## ボクセル化

各姿勢のメッシュは次の手順でボクセル化します。

1. 三角形頂点へ回転を適用する。
2. 回転後bboxの最小点を引き、物体ローカル座標へ移す。
3. bbox寸法とボクセルサイズから物体グリッド寸法を決める。
4. 各三角形のbboxが重なる候補ボクセルを列挙する。
5. 三角形と各ボクセルAABBの交差をSeparating Axis Theoremで判定し、交差する表面ボクセルをmarkする。
6. パディングしたグリッドの外側から6近傍Flood-fillし、外部から到達できないセルを内部として埋める。
7. 表面または内部のセルを物体占有セルとする。

```mermaid
flowchart TD
    A[回転後三角形] --> B[bbox最小点を引いてローカル化]
    B --> C[三角形bboxで候補ボクセルを列挙]
    C --> D[三角形とボクセルAABBの交差判定]
    D --> E[交差する表面ボクセルをmark]
    E --> F[周囲に1セルpadding]
    F --> G[外側から6近傍Flood-fill]
    G --> H[外側から到達不能な空セルを内部化]
    H --> I[占有グリッド OrientedMesh]
```

三角形とボクセルAABBの交差判定は、AABB軸、三角形法線、三角形辺とAABB軸の外積軸を使うSeparating Axis Theoremで行います。これにより、三角形が少しでも通過するボクセルは表面セルとして占有されます。追加の膨張は行いません。ボクセルを大きめに取るほど解は粗くなりますが、三角形通過セルの取りこぼしによる交差見落としは避けます。

## FFT相関による候補評価

配置済み占有グリッドを `O`、配置したい物体の占有グリッドを `P` とします。平行移動 `q` における衝突数は、概念的には次の相関です。

```text
collision(q) = Σ O(x) * P(x - q)
```

全 `q` を直接計算すると、候補数と物体ボクセル数の積になります。実装では `rustfft` で3次元FFTを行い、周波数領域で積を取って逆FFTすることで、全候補の衝突数を一括で得ます。

同時に、配置済みセルとトレイの下側境界からのマンハッタン距離場 `D` を作り、物体との相関を近接スコアとして計算します。

```text
proximity(q) = Σ D(x) * P(x - q)
fit_cost(q) = proximity(q) / occupied_voxel_count(P)
height_cost(q) = height_weight * (q_z / tray_nz)^3
cost(q) = fit_cost(q) + height_cost(q)
```

距離場は「既存物体や下側境界に近いほど小さい」値になります。そのため、`fit_cost` が小さい候補ほど既存の詰まりに寄せられます。高さペナルティは、上方向へ積み上げる配置を必要以上に選ばないための項です。

```mermaid
flowchart TD
    A[配置済みoccupancy] --> B[boolグリッドFFT]
    A --> C[マンハッタン距離場]
    C --> D[距離場FFT]
    E[姿勢ごとの物体occupancy] --> F[物体FFT]
    B --> G[衝突相関<br>IFFT(FFT(O) * conj(FFT(P)))]
    F --> G
    D --> H[近接相関<br>IFFT(FFT(D) * conj(FFT(P)))]
    F --> H
    G --> I[衝突ゼロ候補だけ残す]
    H --> J[近接コスト]
    I --> K[高さペナルティを加算]
    J --> K
    K --> L[最小コスト候補を選ぶ]
```

3次元FFTは、x方向、y方向、z方向の1次元FFTを順に適用して実現しています。逆変換時は `nx * ny * nz` で正規化します。

姿勢ごとの物体ボクセル化、物体FFT、衝突相関、近接相関、到達可能性Flood-fillはRayonで並列評価します。配置済みoccupancyと距離場FFTは状態ごとに1回だけ作り、各姿勢の評価に共有します。beam searchでは同じ物体に対する姿勢ボクセル化結果をbeam状態間で再利用し、論文が重視する「多数の候補姿勢を広く評価する」部分をCPU上でスケールさせています。

## beam search配置

既定の `--beam-width 1` では、各物体について最小コスト候補を1つだけ採用する貪欲配置です。`--beam-width` を2以上にすると、各物体の上位候補を使って複数の部分配置を保持します。

```mermaid
flowchart TD
    A[現在のbeam状態群] --> B[次の物体の上位配置候補を列挙]
    B --> C[候補ごとに配置済み状態を複製]
    C --> D[パック数・メッシュ体積・累積コストで順位付け]
    D --> E[上位beam-width件だけ残す]
    E --> F{未処理物体あり?}
    F -- はい --> B
    F -- いいえ --> G[最良状態を採用]
```

評価は、配置済み物体数を最優先し、同数なら配置済みメッシュ体積、さらに同数なら累積配置コストを比較します。beam幅を広げると、初期の局所最適で後続物体が入らなくなる問題を緩和できますが、状態ごとにFFT探索を行うため計算時間とメモリ使用量が増えます。追加角度姿勢を使う場合でも、最初の24姿勢による基準貪欲経路をbeam内に保護して残し、探索幅を広げたことで既存の軸揃え解が途中で落ちることを避けます。

## 積み込み順探索とBL候補配置

`--strategy order-bl` では、固定順に全グリッド候補をFFT評価する代わりに、beam状態ごとに「次に積む物体」を探索します。各状態では未配置リストの先頭 `--order-window` 個を候補とし、配置に成功した物体を順序から取り除いた状態を次beamへ残します。どの候補も入らない場合に備えて、先頭物体を未配置扱いにして進む枝も作ります。

配置位置はBL法に寄せ、全オフセット空間を走査しません。既存の占有セルの `+x`, `+y`, `+z` 側と原点を前線候補にし、候補を `-z`, `-y`, `-x` の順に動けるだけ詰めます。その後、衝突しないこと、必要なら6軸方向の直線挿入経路が存在することを確認し、`z`, `y`, `x` が小さい候補を優先します。

```mermaid
flowchart TD
    A[beam状態] --> B[未配置先頭order-window個を次物体候補にする]
    B --> C[各物体・姿勢のBL前線候補を生成]
    C --> D[-z, -y, -xへsettle]
    D --> E{衝突なし かつ 直線挿入可能?}
    E -- はい --> F[配置状態をbeam候補へ追加]
    E -- いいえ --> G[候補を破棄]
    F --> H[配置数・未配置数・体積・コストで順位付け]
    G --> H
    H --> I[上位beam-width件だけ残す]
```

この方式は1回の配置評価で見る位置数を `--bl-candidate-limit` に抑えられるため、全グリッド相関より軽い探索を狙えます。一方、候補位置を強く絞るため、FFT全候補評価より良い配置を見落とすことがあります。また順序探索は候補物体数とbeam幅に比例して試行が増えるため、大規模ケースでは `--order-window` と `--beam-width` の調整が重要です。

## Flood-fill到達可能性フィルタ

衝突しない候補でも、既存物体の隙間の奥など、外側から平行移動だけでは到達できない場所にある場合があります。そこで、物体のオフセット空間を作り、衝突しないオフセットを `feasible` として扱います。

実装は、オフセット空間の境界にある `feasible` セルから6近傍Flood-fillし、外側から到達できる候補だけを採用します。

```mermaid
flowchart LR
    A[衝突相関結果] --> B[collision = 0 のオフセット空間]
    B --> C[境界上のfeasibleセルをキューへ投入]
    C --> D[6近傍Flood-fill]
    D --> E[reachable=trueの候補だけ採用]
    B --> F[内部に孤立したfeasible領域]
    F --> G[噛み込みリスクとして除外]
```

この判定は「現在姿勢のまま、グリッド上を平行移動して外側から到達できるか」を厳密に見るものです。任意回転を含む実際の組立・分解経路の完全判定ではないため、連続空間での非噛み込み制約としては改善対象です。

## 連続サブボクセルrefinement

FFT探索はボクセル格子上の離散オフセットを返します。採用後、物体を少しだけ負の `z`、負の `y`、負の `x` 方向へ動かし、ボクセルより細かい単位で詰めます。

手順は次の通りです。

1. 離散オフセットから初期平行移動量を作る。
2. `-z`, `-y`, `-x` の順に移動可能距離を二分探索する。
3. 各方向について探索幅は最大1ボクセル。
4. 12回の二分探索を行い、衝突しない最大距離だけ動かす。
5. この3方向探索を3周繰り返す。

妥当性チェックでは、移動後三角形のbboxがトレイ内に収まること、既存物体と三角形AABB同士が `refine-margin` 付きで重ならないことを確認します。ここでの衝突判定は三角形同士の厳密交差ではなく、三角形AABBの保守的な重なり判定です。そのため、危険な候補を採用しない方向には倒していますが、連続メッシュ同士の真の交差判定は今後追加すべき制約です。

```mermaid
flowchart TD
    A[離散配置] --> B[方向 -z]
    B --> C[1ボクセル幅で二分探索]
    C --> D{トレイ内かつAABB衝突なし?}
    D -- はい --> E[移動量を増やす]
    D -- いいえ --> F[移動量を減らす]
    E --> G[方向 -y]
    F --> G
    G --> H[方向 -x]
    H --> I[3周繰り返す]
    I --> J[refined配置]
```

refinement後の物体は、最終的な平行移動量で三角形を変換して出力します。占有セルは、refinement後の位置に対応する範囲へ再stampします。

## ray-casting分解可能性解析

配置完了後、最終トレイの占有セルを使って、直線方向に取り出せる物体があるかを解析します。方向は `+x`, `-x`, `+y`, `-y`, `+z`, `-z` の6方向です。

各方向について、同じ走査線上に現れる物体の順序からDirectional Blocking Graphを作ります。走査方向に見て手前の物体が奥の物体を遮る場合、手前から奥へ有向辺を張ります。強連結成分を計算し、単独成分かつ自己ループがない物体は、その方向へ取り出せるものとして取り除きます。

```mermaid
flowchart TD
    A[最終occupancy<br>セルごとに所有物体IDを保持] --> B[6方向を順に走査]
    B --> C[走査線上の物体出現順を抽出]
    C --> D[Directional Blocking Graphを構築]
    D --> E[Tarjan法で強連結成分を計算]
    E --> F{単独成分かつ自己ループなし?}
    F -- はい --> G[その方向に取り出し可能として除去]
    F -- いいえ --> H[残留候補]
    G --> I{全物体を除去?}
    I -- はい --> J[全物体取り出し可能]
    I -- いいえ --> B
    H --> K[残った強連結成分を噛み込み候補として報告]
```

この解析は、配置後の状態を評価するものです。噛み込み候補が残った場合は、次の取り外し・再挿入後処理の入力になります。

## 取り外し・再挿入後処理

ray-casting分解可能性解析で強連結成分として残ったグループは、直線方向の取り出し順序が見つからない噛み込み候補です。後処理では、そのグループを一度トレイから取り外し、残った物体を固定したまま再挿入します。

```mermaid
flowchart TD
    A[ray解析で残った強連結グループ] --> B[対象グループを取り外す]
    B --> C[残った物体だけでoccupancyを再構築]
    C --> D[再挿入順序を複数生成]
    D --> E[元配置を避ける候補も試す]
    E --> F[高さ重みを変えて再パック]
    F --> G[再度ray解析]
    G --> H{残留物体数または残留グループ数が改善?}
    H -- はい --> I[再挿入結果を採用]
    H -- いいえ --> J[元の配置を維持]
    I --> K{最大パス数に到達?}
    J --> K
    K -- いいえ --> A
    K -- はい --> L[最終配置]
```

再挿入では、対象グループの元順、逆順、bbox体積の昇順・降順を試します。また、高さペナルティ係数を複数倍率で試し、同じ衝突・到達可能性・refinement制約の中で再配置します。採用条件は安全側で、ray分解スコアが改善した候補だけを最終配置へ反映します。

ray分解スコアは、残った強連結成分に含まれる物体数を第一優先、残ったグループ数を第二優先で比較します。改善しない再挿入や、取り外した物体を全て再挿入できない候補は破棄します。これにより、後処理が既に成立している制約やパック数を悪化させないようにしています。

## 未配置物体向け局所再パック

ray解析で噛み込みが無い場合でも、貪欲配置が局所的に詰まり、未配置物体が残ることがあります。`--repack-passes` が1以上の場合は、配置済み物体の一部を取り外し、未配置物体を含めて再挿入する局所再パックを試します。

```mermaid
flowchart TD
    A[未配置物体リスト] --> B[末尾配置・上側配置・小体積配置から取り外し候補を作る]
    B --> C[候補グループを一度取り外す]
    C --> D[未配置物体と取り外した物体を複数順序で再挿入]
    D --> E{配置数または配置済み体積が改善?}
    E -- はい --> F[候補を採用し未配置リストを更新]
    E -- いいえ --> G[元配置を維持]
    F --> H{最大パス数に到達?}
    G --> H
    H -- いいえ --> B
    H -- はい --> I[最終配置]
```

取り外し候補は、後から置いた物体、bbox上端が高い物体、体積が小さい物体から作ります。採用条件は安全側で、配置済み物体数が増える、または同数でも配置済みメッシュ体積が増える場合だけ反映します。

## Thingi10Kサンプルとbboxタイト検証

Thingi10Kサンプルは、公式APIからメタデータとSTLリンクを取得し、扱いやすいサイズへ正規化して保存します。各ケースには `attribution.json` を置き、ライセンス・作者・出典・元の面数などを保持します。

検証スクリプトは、各物体のbboxから次の値を計算します。

| 指標 | 意味 |
| --- | --- |
| `sum_bbox_volume` | 各物体bbox体積の合計 |
| `sum_bbox_xy_footprint` | 各物体bboxのx/y底面積合計 |
| `max_bbox_extent` | 各軸について最大のbbox寸法 |

トレイは、最大物体が入る寸法を確保しつつ、底面積を `sum_bbox_xy_footprint` より小さくするように探索します。これにより、単純に全物体を平置きするだけでは満たせない条件になります。

大規模ベンチマーク用の `stacked_mixed_10x` は、`stacked_mixed` の36個を10回複製した360物体ケースです。通常ケースと同じくbboxからトレイを計算しますが、底面fractionを10分の1にし、元ケースに近い底面へ高さ方向に積む条件にしています。STL本体は重複保存を避けるため、可能なら相対シンボリックリンクで作成します。

```mermaid
flowchart TD
    A[Thingi10K STL群] --> B[各物体のbbox寸法を計算]
    B --> C[底面fractionと体積slackを組み合わせ]
    C --> D[トレイ寸法をvoxel単位に切り上げ]
    D --> E[spectral-packing packを実行]
    E --> F{全物体をパックできた?}
    F -- はい --> G[validation.jsonへ結果保存]
    F -- いいえ --> H{single-attempt?}
    H -- はい --> G
    H -- いいえ --> C
```

`--time-limit-seconds` を指定した検証では、CLI側の時間制限により途中で止まった場合も、配置済み物体数、トレイ寸法、密度、ログパスを `validation.json` に記録します。`--single-attempt` を併用すると、最初のbboxタイトトレイだけを試し、時間比較用の部分結果を保存します。

## 計算量

以降の記号を使います。

| 記号 | 意味 |
| --- | --- |
| `M` | 入力物体数 |
| `R` | 試す姿勢数。24超では追加角度姿勢を含む |
| `T = nx * ny * nz` | トレイのボクセル数 |
| `C_r` | ある姿勢での有効な平行移動候補数 |
| `V_r` | ある姿勢での物体bbox内ボクセル数 |
| `F_i` | 物体 `i` の三角形数 |
| `B_f` | ある三角形のbboxが重なる候補ボクセル数 |
| `B` | beam searchで保持する部分配置数 |
| `W` | `order-bl` で各状態から試す次物体候補数 |
| `L` | `order-bl` で評価するBL前線候補数 |
| `E` | ray blocking graphの辺数 |

| 処理 | 時間計算量の目安 | メモリ計算量の目安 | 備考 |
| --- | ---: | ---: | --- |
| 入力ソート | `O(M log M)` | `O(M)` | bbox体積の降順 |
| STL読み込み | `O(ΣF_i)` | `O(ΣF_i)` | 三角形をメモリ保持 |
| 1姿勢のボクセル化 | `O(Σ B_f + V_r)` | `O(V_r)` | `B_f` は三角形bboxが重なる候補ボクセル数。各候補でSAT交差判定を行う |
| 距離場作成 | `O(T)` | `O(T)` | 6近傍BFS |
| 配置済み/距離場FFT | `O(T log T)` | `O(T)` | 物体1個の探索ごとに作成 |
| 1姿勢の相関評価 | `O(T log T + C_r)` | `O(T)` | 物体FFT、衝突IFFT、近接IFFT、候補走査 |
| 到達可能性Flood-fill | `O(C_r)` | `O(C_r)` | `--no-interlock` で省略可 |
| 1物体の配置探索 | `O(T log T + R * (voxelize + T log T + C_r))` | `O(T + R * max V_r)` | 姿勢ごとのボクセル化と相関評価はRayonで並列化 |
| beam search | `O(B * (T log T + R * (T log T + C_r)) + R * voxelize)` | `O(B * (T + 配置済み情報) + R * max V_r)` | 同一物体の姿勢ボクセル化結果はbeam状態間で再利用。基準貪欲経路保護で追加1候補を扱うことがある |
| order-bl順序探索 | `O(B * W * R * L * V_r)` | `O(B * T + キャッシュ済み姿勢)` | 全グリッド走査を避けるが、順序候補数に比例する |
| refinement | 最悪 `O(108 * Σ_{j<i}(F_i * F_j))` | `O(F_i)` | 実際はbboxで大きく枝刈り |
| ray分解解析 | `O(P * (T + E + M))` | `O(T + E + M)` | `P` は最大18方向走査程度 |
| 取り外し・再挿入後処理 | `O(K * G * A * 再挿入探索)` | `O(T + ΣF_i)` | `K` は後処理パス数、`G` は残留グループ数、`A` は試行する順序・重みの組み合わせ数 |
| 未配置局所再パック | `O(K * G * A * 再挿入探索)` | `O(T + ΣF_i)` | `K` はrepackパス数、`G` は取り外し候補グループ数 |
| 時間制限チェック | 各長時間ループに定数時間の追加 | `O(1)` | 配置探索、refinement、再挿入処理の途中で確認する |

FFT相関により、各候補ごとに物体ボクセルを重ねて衝突数を数える `O(C_r * V_r)` を避け、トレイ全体の `O(T log T)` に寄せています。一方、CPU実装であり、各物体・各姿勢に対してトレイサイズのFFT配列を扱うため、`--voxel` を小さくすると急速に重くなります。

### メモリ使用の特徴

探索中は、トレイサイズ `T` の配列を複数持ちます。

| 配列 | 型 | 用途 |
| --- | --- | --- |
| `occupied` | `bool` | 配置済みセル |
| `distance` | `f32` | マンハッタン距離場 |
| `existing_fft` | `Complex32` | 配置済みセルのFFT |
| `distance_fft` | `Complex32` | 距離場のFFT |
| `object_fft` | `Complex32` | 物体占有のFFT |
| `collision`, `proximity` | `f32` | 逆FFT後の相関結果 |

そのため、実効メモリは概ね `T` に比例します。`Complex32` は実部・虚部を持つため、同じ要素数の `f32` 配列より大きくなります。

## 考慮している制約

| 制約 | 現状の扱い |
| --- | --- |
| トレイ境界 | bboxとボクセル寸法で、配置候補がトレイ内に収まるものだけ採用 |
| 物体同士の衝突 | 保守ボクセル化した占有セルのFFT相関で排除 |
| 外から到達できない配置 | オフセット空間のFlood-fillで排除 |
| 配置順序の局所最適 | spectral beam search、order-bl順序探索、未配置局所再パックで一部を詰め直す |
| 高さ方向の積み上げ | `height_weight * q_z^3` で抑制 |
| サブボクセルの隙間詰め | 負の3軸方向へ二分探索 |
| refinement時の安全余白 | 三角形AABBに `refine-margin` を加えて保守的に判定 |
| 最終的な取り出しやすさ | ray-casting blocking graphで後解析 |
| 噛み込み候補の改善 | ray解析で残ったグループを取り外し・再挿入し、改善した候補だけ採用 |
| 実行時間の上限 | `--time-limit-seconds` 到達後は新規探索を止め、配置済み部分結果を保存 |
| ライセンス・出典 | Thingi10Kケースごとに `attribution.json` を保存 |

この表のうち、現在の実装が「パック済み」として採用するかどうかの直接条件にしているのは、トレイ境界、保守ボクセル衝突、離散到達可能性、refinement時のAABB安全余白です。任意経路での分解可能性は、制約として重要ですが、現状では完全には判定できていないため積み残しに分類します。

## 積み残しと限界

| 項目 | 現状 | 影響 |
| --- | --- | --- |
| GPU高速化 | 未実装 | 論文のスケーラビリティには届かず、大きなトレイや細かいボクセルで遅くなる |
| 完全な全体最適化 | 限定的 | beam searchと局所再パックは実装済みだが、幅・候補・パス数は有限で、全配置空間の最適性は保証しない |
| 任意角度回転 | 限定的 | 追加角度姿勢は実装済みだが、連続的な全角度探索ではない |
| 連続メッシュ同士の最終検査 | 未実装 | 保守ボクセル化とrefinement時AABBで安全側に扱うが、最終出力に対する独立した三角形交差レポートはまだない |
| 多段・全体最適化 | 限定的 | beam searchと局所再パックは有限幅・有限パスのため、先に置いた物体の選択が後続物体を詰めにくくする場合がまだある |
| 複数トレイ | 未実装 | 1つの直方体トレイに入る範囲だけを扱う |
| 並列化 | 実装済み/限定的 | 姿勢ごとのボクセル化・FFT候補評価はCPU並列化済み。GPUや分散実行は未実装 |
| 非watertightメッシュへの堅牢性 | 限定的 | 内部充填が外部Flood-fillに依存するため、穴あきメッシュでは占有推定が不安定になりうる |
| 実分解経路の完全性 | 限定的 | ray解析は6軸直線移動ベースで、回転や斜め方向の取り出しは見ない |

## 実装上の設計判断

1. **制約充足を優先する。** 性能が論文実装に劣っても、パック済みとして出力する候補が実装上の制約モデルを破らないことを優先する。
2. **配置探索はFFTで広く、必要に応じてbeamで残す。** 既定は従来通り貪欲だが、`--beam-width` で複数の部分配置を保持できる。
3. **非噛み込み性は2段で見る。** 配置時にFlood-fill到達可能性で危険候補を減らし、配置後にray-castingで直線分解可能性を報告する。
4. **後処理は改善した場合だけ採用する。** 取り外し・再挿入でray分解スコア、配置数、配置済み体積が改善しない候補は破棄する。
5. **refinementは安全側に倒す。** 厳密交差ではなくAABB重なりを使うため、過剰に衝突扱いすることがあるが、危険な重なりを出しにくい。
6. **時間制限でも部分結果を残す。** 大量物体ケースでは探索が長くなるため、打ち切りを失敗扱いにせず、評価可能なSTLとJSONを残す。
7. **サンプルは再現可能性を優先する。** Thingi10Kの選択モデル、正規化寸法、bboxタイト条件をスクリプトに固定し、検証結果をJSONとして保存する。

## 改善候補

| 優先度 | 改善案 | 期待効果 |
| --- | --- | --- |
| 高 | 配置状態をまたぐFFT配列キャッシュ | 同じoccupancyに対する再計算削減 |
| 高 | 並列化粒度の調整 | Rayonのスレッド利用効率を改善 |
| 高 | GPU FFT / GPU相関 | 論文に近いスケールへ拡張 |
| 中 | 三角形AABB階層 | refinementと最終検査の速度を改善 |
| 中 | 後処理の候補生成拡張 | 局所再パックで試す領域、順序、評価関数を増やす |
| 中 | 追加角度姿勢の候補選別 | 軸揃えでは入らない物体へ、過剰な探索増を避けながら対応 |
| 低 | 複数トレイ対応 | 大量物体のバッチ配置 |
| 低 | ビューア上で個別物体色分け | GitHub Pagesでの確認性向上 |

## 関連ファイル

| ファイル | 内容 |
| --- | --- |
| `src/main.rs` | CLI、STL処理、ボクセル化、FFT探索、refinement、ray解析、サンプル生成 |
| `scripts/fetch_thingi10k_cases.py` | Thingi10Kモデル取得、正規化、帰属情報生成 |
| `scripts/create_thingi10k_benchmark_case.py` | 既存Thingi10Kケースを複製し、大規模ベンチマークケースを生成 |
| `scripts/validate_thingi10k_tight.py` | bboxタイト条件の探索と検証 |
| `samples/thingi10k/validation.json` | 検証結果 |
| `docs/index.html`, `docs/app.js` | GitHub Pages用ビューア |
| `docs/assets/results.json` | ビューアへ渡す公開結果メタデータ |
| `.github/workflows/pages.yml` | `docs/` をGitHub PagesへActionsデプロイするworkflow |
