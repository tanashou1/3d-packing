...
| 厳密に満たす対象 | refinement時の保守的な非衝突 | 三角形SAT判定で安全余白込みの重なり候補を採用しない |
...
| `--refine-margin` | refinement中の三角形クリアランス |
...
妥当性チェックでは、移動後三角形のbboxがトレイ内に収まること、既存物体と三角形同士が `refine-margin` 付きで重ならないことを確認します。三角形同士の判定は、三角形法線、辺同士の外積軸、coplanar時の面内分離軸を使うSATです。refinement後の占有セルは、移動後三角形をトレイ格子上で再ボクセル化して更新します。これにより、subvoxel移動した物体の古いローカル占有ボクセルが余分なセルへ膨らみ、後続配置を不必要に遠ざけることを避けます。
...
| beam search | `O(B * 1物体の配置探索)` | `O(B * (T + 配置済み情報))` | `--beam-width` に比例。基準貪欲経路を保護するため、追加で1候補を扱うことがある |
...
| 配置順序の局所最適 | beam searchで複数の部分配置を保持し、未配置局所再パックで一部を詰め直す |
| 高さ方向の積み上げ | `height_weight * q_z^3` で抑制 |
| サブボクセルの隙間詰め | 負の3軸方向へ二分探索 |
| refinement時の安全余白 | 三角形SAT判定に `refine-margin` を加えて保守的に判定 |
...
| `scripts/fetch_abc_step_subset.py` | HuggingFace上のABC STEPサブセットを取得し、`gmsh` でSTL化して指定個数のケースを生成 |
| `scripts/create_abc_benchmark_cases.py` | ローカルABC Dataset OBJ/STLからCADベンチマークケースを生成 |
| `scripts/publish_benchmark_results.py` | 検証済みSTL/JSONを `docs/assets` に反映し、ビューア結果一覧を更新 |
| `scripts/validate_thingi10k_tight.py` | bboxタイト条件の探索と検証。外部 `case_config.json` も読み込める |
